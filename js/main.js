/* ═══════════════════════════════════════════════════════════
   VerfiX main.js — Production Build
   verfix.ch · Swiss Trust Infrastructure
═══════════════════════════════════════════════════════════ */

/* ─── CONFIG ─── */
var FORMSPREE_CONTACT = 'YOUR_FORMSPREE_ID'; // Replace: https://formspree.io/f/XXXXXXXX
var FORMSPREE_DEMO    = 'YOUR_FORMSPREE_ID'; // Same or separate form ID
var GA4_ID     = 'G-XXXXXXXXXX';   // Replace with real GA4 Measurement ID
var GSC_VERIFY = '';                // Google Search Console verification code
...
function nav(id) {
  ...
  if (t) { t.classList.add('on'); curPage = id; }
  else    { document.getElementById('pg-home').classList.add('on'); curPage = 'home'; }
  window.scrollTo(0, 0);
  closeAll(); closeMob();
...
/* ─── MEGA MENU — hover + click, mouseleave auto-close ─── */
var _openM = null, _menuTimer = null;
...
function toggleMenu(id) {
  ...
  closeAll();
  ni.classList.add('open'); _openM = id;
}

function _openHover(id) {
  if (_menuTimer) { clearTimeout(_menuTimer); _menuTimer = null; }
  if (_openM === id) return;
  closeAll();
  var ni = document.getElementById('ni-' + id);
  if (ni) { ni.classList.add('open'); _openM = id; }
}

function _startClose() {
  _menuTimer = setTimeout(function(){ closeAll(); }, 150);
}
...
function closeAll() {
  if (_menuTimer) { clearTimeout(_menuTimer); _menuTimer = null; }
  document.querySelectorAll('.ni.open').forEach(function(el){ el.classList.remove('open'); });
  _openM = null;
}
...
function _initMenuHover() {
  document.querySelectorAll('.ni[id]').forEach(function(ni) {
    ...
    ni.addEventListener('mouseenter', function(){ _openHover(id); });
    ni.addEventListener('mouseleave', function(){ _startClose(); });
    var mega = ni.querySelector('.mega');
    if (mega) {
      mega.addEventListener('mouseenter', function(){ _cancelClose(); });
      mega.addEventListener('mouseleave', function(){ _startClose(); });
    }
  });
}
